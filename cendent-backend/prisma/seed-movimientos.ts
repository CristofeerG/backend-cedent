...
function conHoraAleatoria(fecha: Date): Date {
  const d = new Date(fecha);
...
async function simularMovimientos() {
  ...
  if (!sucursales.length) { console.error('Sin sucursales. Abortando.'); return; }

  const kits = await prisma.kits.findMany({ include: { detalle_kit: true } });
  if (!kits.length) { console.error('Sin kits. Abortando.'); return; }

  const todosUsuarios = await prisma.usuarios.findMany();
  if (!todosUsuarios.length) { console.error('Sin usuarios. Abortando.'); return; }

  // 3. Iterar por sucursal ──────────────────────────────────────────────────────
  ...
  for (const sucursal of sucursales) {
    ...
    if (!lotes.length) { console.log('  Sin lotes. Saltando.'); continue; }

    // Mapa id_producto → array de lotes (orden FEFO = vencimiento ascendente)
    const lotesPorProducto = new Map<number, typeof lotes>();
    for (const lote of lotes) {
      if (lote.id_producto === null) continue;
      if (!lotesPorProducto.has(lote.id_producto)) lotesPorProducto.set(lote.id_producto, []);
      lotesPorProducto.get(lote.id_producto)!.push(lote);
    }
    // Lista plana de ids de productos disponibles
    ...
    const kitsValidos = kits.filter((k) =>
      k.detalle_kit.some((d) => d.id_producto !== null && lotesPorProducto.has(d.id_producto)),
    );

    console.log(
      `  Lotes: ${lotes.length} | Productos con lote: ${idsProductosDisponibles.length} | ` +
    ...
    for (let offset = 0; offset <= diasTotal; offset++) {
      ...
      if (!esDiaLaboral(fechaDia)) continue;

      // Intensidad semanal variable (semana 0,3,6,… es "semana baja")
      const semana = Math.floor(offset / 7);
      const esSemanaAlta = semana % 3 !== 0;

      const registrosDia: Insert[] = [];
      ...
      if (kitsValidos.length) {
        ...
        for (let k = 0; k < cantKits; k++) {
          ...
          for (const detalle of kit.detalle_kit) {
            if (detalle.id_producto === null) continue;
            const lotesProducto = lotesPorProducto.get(detalle.id_producto);
            if (!lotesProducto?.length) continue;
      ...
      if (idsProductosDisponibles.length) {
        ...
        for (const idProducto of productosDelDia) {
          ...
          // Cantidad directa: entre 1 y 5 unidades con variación
      ...
      // Garantía mínima: si el día quedó por debajo de 25 registros, completar con directos
      if (idsProductosDisponibles.length) {
    ...
    // 5. Insertar en batches de 300 ────────────────────────────────────────────
    const BATCH = 300;
    let insertadosSucursal = 0;
    for (let i = 0; i < inserts.length; i += BATCH) {
      await prisma.movimientos.createMany({ data: inserts.slice(i, i + BATCH) as any });
